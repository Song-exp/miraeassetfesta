...
4. 🔴 **gold 파일(`eval/*.jsonl`)을 고치지 마라.** gold 결함으로 판정이 불가한 문항은 `docs/gold_defects_2026-09-03.md` 에
   목록이 있다 — 그 문항들은 **판정 보류**로 표시하고 ✅/❌ 어느 쪽으로도 세지 않으며, 집계 분모에서 뺀다.
   새 gold 결함을 찾으면 그 문서에 항목만 추가한다. **gold 수정 근거로 "서버 답과 일치"를 쓰지 마라** —
   계측기를 피측정 대상에 맞추면 측정이 사라진다. 근거는 질문 문언·도메인 정본·DB 실측 셋뿐이다.
   단 **그 문항에서 관찰한 서버 결함은 정상적으로 §③에 올린다** — gold 가 못 쓴다고 서버 결함까지 덮지 않는다.
