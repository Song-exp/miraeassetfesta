...
**⑤ 서버는 GitHub 에서 당겨옵니다 — 내 워킹트리를 보지 않습니다.**
`deploy.sh` 의 코드·yaml 반영은 서버에서 `git pull --ff-only` 를 돌리는 것입니다.
**push 하지 않은 수정은 어떤 배포 모드로도 반영되지 않습니다.** 로컬 → 서버 직접 전송은 DB(`scp`) 하나뿐이고,
`.env` 는 아예 전송되지 않습니다(서버에만 존재). 이미지도 서버에서 빌드합니다.
반대로 `shared/*.yaml` 은 파일을 보내도 반영되지 않습니다 — 개체는 DB 의 `kg_*` 로 구워지므로 `--db-only` 가 필요합니다.
→ 전달 경로 전체는 `docs/DEPLOY.md` **§0-B**, KG 갈래는 **§0-C**.
