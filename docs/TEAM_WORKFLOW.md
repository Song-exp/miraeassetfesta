...
```bash
git add ontology/... && git commit && git push      # 🔴 push 가 먼저입니다
```
...
### 🔴 왜 push 가 먼저인가 — 서버는 내 PC 를 보지 않습니다

**배포는 내 파일을 서버로 보내는 작업이 아닙니다.** 서버가 **GitHub 에서 당겨옵니다**(`git pull --ff-only`).
로컬에서 서버로 직접 가는 것은 **DB 파일 하나뿐**입니다(`scp`, `--db-only`/전체 배포에서만).

```
내 PC  ──push──▶  GitHub  ──pull──▶  NCP 서버        (코드·yaml·문서)
내 PC  ─────────── scp ────────────▶  NCP 서버        (DB 263MB, 그때만)
```
...
| 규칙 | 뜻 |
| :-- | :-- |
| **커밋·push 안 한 수정은 서버에 없습니다** | "고쳤는데 왜 그대로지" 의 1순위 원인입니다. `git status` 부터 보세요 |
| 남이 편집 중인 파일도 **커밋 전이면 배포에 안 실립니다** | 동시 작업 중에도 배포가 안전한 이유입니다 |
| `.env`(HCX 키·토큰)는 **전송되지 않습니다** | 서버에만 있습니다. 로컬 `.env` 를 고쳐도 서버는 안 바뀝니다 |
| 이미지는 **서버에서 빌드**됩니다 | 로컬에 Docker 가 없어도 배포는 됩니다 |

> 전달 경로 전체 그림과 프리즈 주의사항은 **`docs/DEPLOY.md` §0-B·§0-C**.
> 배포 실행은 SSH(22번)가 열린 자리에서만 됩니다 — ACG 가 리드 PC IP 에 고정. 안 되면 담당자에게.
