...
## 4. ~~📮 주최측 문의사항 (디스코드)~~ → 🔴 **철회 (2026-08-31)**

> **주최가 답하지 않기로 확정했다.**
> 2026-08-31 15:02 디스코드 — *"코드 및 기호등의 정의에 대해서는 말씀드리지 않는 것으로 확인드렸습니다.
> 이 부분 직접 확인 및 판단하셔서 구분하시면 됩니다."*
> 8/24 공지 §0-B(*"코드 값 테이블을 드릴 수는 없을 것으로 보입니다"*)의 연장이다. → [`qna_discord_2026-08-31.md`](../qna_discord_2026-08-31.md) §1
>
> 원래 방침은 *"컬럼을 정본으로 두고 주최에 문의"* 였다. 뒷부분이 없어졌으므로 **앞부분으로 종결한다** —
> **컬럼(`wu_inv_ast_type`)이 정본이다.** 근거는 이 문서 §3 실측 그대로다:
> 컬럼 매핑 92% 가 최고치이고 상품명 키워드로 바꿔도 92% 로 안 오른다.
> 남은 오분류는 `TIGER 우선주`(KR7261140008) **1건**뿐이라 `query_rules.자산군오분류_제외` 로 종목코드를 지정해 뺐다.
>
> 🔴 같은 유형이 또 나오면 **문자열 규칙을 새로 만들지 말고 종목코드를 추가**한다(서현 지적 반영).

아래는 보낼 예정이었던 원문. 기록용으로 남긴다.
