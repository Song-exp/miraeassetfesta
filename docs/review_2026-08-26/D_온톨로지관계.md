...
| D-1-07 | 펀드 `derivation_rules.assetClass` | 규칙이 오탐·누락을 얼마나 내는가 · 답변에 ‘추정’ 을 병기하는가 | {'축': '무엇에 투자하는가 (하위클래스 결정)', '규칙': '① zrin_btyp_nm(18종, 국내/해외 내장) ② 없으면 종목명 괄호 표기 — (주식…)→Equity, (채권…)→Bond, (주식혼합…)→EquityMixed, (채권혼합…)→BondMixed ③ 없으면 or_attr_desc 폴백', '판정률': '미산출(2차) — zrin_btyp_nm 보유 11,281/23,676 · 판매중 8,551/10,962'} | ⚠️ | **규칙 ②(괄호 표기)를 2차 데이터로 처음 실측했다.** 괄호 있는 종목명 14,277/23,676(판매중·공모 8,705/8,969) · 자산군성 토큰 **101종**. 규칙 ② 커버 11,802행 · `zrin_btyp_nm` 과 둘 다 있는 9,827행 중 **불일치 1,533(15.6%)** — 오탐이 아니라 **층위 차이**다: 괄호는 약관상 분류, 제로인은 실질 분류(`[채권혼합-재간접형]`→해외주식혼합형 · `(주식)` 롱숏→주식혼합형). 규칙 ①(zrin 우선) 순서는 옳다. 폴백 효과는 작다 — zrin 결측 12,395 중 1,975행, 판매중·공모 418 중 262행. 규칙 ②가 못 읽는 토큰: `ELS-파생형` 458 · `혼합-재간접형` 284 · `DLS-파생형` 188 · `재간접형` 155 · `국공채` 55. 🔴 규칙 `자산군_주식형` 은 `'%(주식%'` 소괄호만 봐서 `[주식…]` 대괄호 표기(5,238행 중 일부)를 놓친다 → **`query_rules.이름표기_우선순위` 신설**(자산군은 zrin 우선 · 괄호는 "약관 분류 기준" 병기, 2026-08-30). 파생 컬럼 방식은 같은 날 저녁 리드 결정으로 폐기 — 규칙은 조건식으로만. **미결(리드)**: ELS/DLS/혼합/재간접형 토큰을 어느 AssetClass 로 볼지 · `자산군_주식형` 의 대괄호 보강. 기록 `공모펀드_검토기록_2026-08-30_이름계열.md` §3.6·§4 |
| D-1-08 | 펀드 `derivation_rules.isFundOfFunds` | 규칙이 오탐·누락을 얼마나 내는가 · 답변에 ‘추정’ 을 병기하는가 | {'축': '운용 구조 (자산군과 직교)', '규칙': "zrin_attr_nms 태그 'FoFs'(M112) 또는 종목명 '재간접' 또는 or_attr_desc='재간접' (보조: zrin_fd_cmst_rt 가 높음)"} | ✅ | 이름 `재간접` 3,803행 중 `or_attr_desc='재간접'` 3,522 · **컬럼만 130** · 태그 `FoFs` 2,052 → "태그·컬럼 우선, 종목명 보조" 순서가 실측으로 맞다. 종목명 항은 `itm_nm LIKE '%재간접%'` 조건식 그대로(`query_rules.이름표기_우선순위` 가 컬럼 > 이름 순서를 명시). 판매중·공모 2,684. '추정' 병기는 컬럼·태그 없이 이름만으로 판정한 경우에 한정. 기록 §3.6 |
| D-1-09 | 펀드 `derivation_rules.usesDerivatives` | 규칙이 오탐·누락을 얼마나 내는가 · 답변에 ‘추정’ 을 병기하는가 | {'축': '파생 활용 (직교)', '규칙': "종목명에 '파생' 또는 or_attr_desc='파생상품'", '해당': '2302 이상(or_attr_desc 기준)'} | ✅ | 이름 `파생|ELS|DLS` 2,210행 중 `or_attr_desc='파생상품'` 2,024 · **컬럼만 278** → OR 규칙 유지가 맞다(이름만으로는 278행 누락). 판매중·공모는 332행뿐(파생형은 대부분 판매완료 ELS/DLS). 기록 §3.6 |
| D-1-10 | 펀드 `derivation_rules.isMasterFeeder` | 규칙이 오탐·누락을 얼마나 내는가 · 답변에 ‘추정’ 을 병기하는가 | {'축': '모자형 구조 (직교)', '규칙': "zrin_attr_nms 태그 '자펀드'(M109) > 종목명 '증권자'/'자투자신탁' 패턴 > 1차 설명서 플래그(external_facts.is_masterfeeder)"} | ✅ | 이름 `자투자신탁/증권자` 7,746행 중 태그 `자펀드` 6,879(89%) · **태그만 95** → 태그 > 종목명 순서가 맞다. 🔴 함정 실측: `모투자신탁` 을 substring 으로 잡으면 **`사모투자신탁` 1,969행이 모펀드로 오판**된다 — 경계식 `(?<!사)모투자신탁` / SQL 로는 `itm_nm NOT LIKE '%사모투자신탁%' AND itm_nm LIKE '%모투자신탁%'` 이면 1행(`OK후순위채모투자신탁B2호`)뿐. 모펀드는 마스터 밖 개체(`MotherFund_*`, `feedsInto` 1,704)이므로 이름 규칙은 자펀드 판정에만 쓴다. 기록 §3.6 |
...
| D-2-11 | 펀드 `ETF제외` | `validate_sql` 이 차단하는가, 프롬프트 문구뿐인가 | itm_nm NOT LIKE '%상장지수%' | ⚠️ | `validate_sql` 은 차단하지 않는다 — **프롬프트 규칙뿐**이다(`_FORBIDDEN`·테이블 허용만 검사). 다만 236행 **전부 판매완료**라 `기본모수`(판매중 AND 공모) 필터가 걸리면 자동 배제된다. 규칙 표현은 `itm_nm NOT LIKE '%상장지수%'` 유지(2026-08-30 저녁 결정: 이름 규약은 파생 컬럼이 아니라 조건식). 회귀 테스트는 없다 — "펀드 순자산 Top10" 류 gold 문항에 `NOT LIKE '%상장지수%'` 포함 여부를 점검하는 문항 신설 권고(P1-1). 기록 §3.6 |
