...
③ 9/3 재검증 — 접지·조인 성공(위 trace). 잔여: 지역 필터 중첩이 16→9건으로 잘라 `지역질의_합집합` 에 "구성종목 근거가 있으면 지역 필터를 겹치지 않는다" 보강(진짜 중국 상품 5종이 rgn 오분류로 잘림 실측). 재확인 완료(9/4 서버): 합집합 규칙 적용으로 **9건 → 14건** 회복, 1위 'RISE 차이나AI반도체TOP4Plus' 13.25% — gold 와 비중·순위 일치. 잔차 2건은 글로벌 로봇 ETF 라 "중국 반도체" 취지상 제외가 자연스럽다고 판정. 형의 결정층 가드(기본모수 기계 주입 · 모수 한정 고지)와 규칙층이 함께 작동한 종결 사례.
