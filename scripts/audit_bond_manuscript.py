...
chk(L, "등급일 있는 행(§1.2)", 17875, one(f"SELECT COUNT(*) {B} WHERE crd_grd_dt IS NOT NULL AND crd_grd_dt<>0"))
chk(L, "등급일=발행일 행(§1.2)", 13996, one(f"SELECT COUNT(*) {B} WHERE crd_grd_dt IS NOT NULL AND crd_grd_dt<>0 AND crd_grd_dt=isu_dt"))
chk(L, "표준코드(ISIN) 컬럼 수(§1.2)", 0, sum(1 for d in c.execute(f"SELECT * {B} LIMIT 1").description if "isin" in d[0].lower()))
