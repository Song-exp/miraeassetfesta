...
chk("C-scls-4", "국고채 371·국민주택 210·지역개발 1,679·도시철도 452·물가채 6·중앙은행채 33", f"SELECT SUM(TRIM(std_pd_scls_nm)='국고채'), SUM(TRIM(std_pd_scls_nm)='국민주택채'), SUM(TRIM(std_pd_scls_nm)='지역개발채'), SUM(TRIM(std_pd_scls_nm)='도시철도채'), SUM(TRIM(std_pd_scls_nm)='물가채'), SUM(TRIM(std_pd_scls_nm)='중앙은행채') {B}", (371, 210, 1679, 452, 6, 33))
...
chk("C-mkt-1", "장외 avg applied_yield 8.59 · 장내 4.13", f"SELECT ROUND(AVG(CASE WHEN pd_exg_mkt='장외' THEN applied_yield END),2), ROUND(AVG(CASE WHEN pd_exg_mkt='장내' THEN applied_yield END),2) {B}", (8.59, 4.13), 0.01)
...
chk("C-inrt-3", "이름 '변' 431행: 변동 393 · 고정+변동 37 · 고정 1", f"SELECT COUNT(*), SUM(bd_inrt_tcd='변동금리'), SUM(bd_inrt_tcd='고정+변동금리'), SUM(bd_inrt_tcd='고정금리') {B} WHERE pd_nm REGEXP '[(/]변[)/]'", (431, 393, 37, 1))
...
chk("C-srfc-2", "srfc=0 중 CB 이름 324 · 주식연계 합계 489", f"SELECT SUM(pd_nm REGEXP '\\d+CB|[(/]전환[)/]'), SUM(pd_nm REGEXP '\\d+CB|[(/]전환[)/]|\\d+EB|\\d+BW|교환|신주') {B} WHERE srfc_irt=0", (324, 489))
...
for g, n in [("AAA",8722),("AA+",3530),("AA0",1241),("AA-",2543),("A+",678),("A0",737),("A-",124),("BBB+",109),("BBB0",45),("BBB-",7),("BB0",13),("BB-",3),("B+",2),("B-",5),("C0",103)]:
...
# ── 위험지표 ─────────────────────────────────────────────
chk("C-dur-1", "dur NULL 16 · 99 3 · 0 52 (만기도래 48 · 잔존>0 4) · max ≤56", f"SELECT SUM(dur IS NULL), SUM(dur=99), SUM(dur=0), SUM(dur=0 AND remaining_days=0), SUM(dur=0 AND remaining_days>0), ROUND(MAX(CASE WHEN dur<>99 THEN dur END),1) {B}", (16, 3, 52, 48, 4, 56.0), 1.0)
chk("C-dur-2", "평균 dur 국공채 3.97 > 특수채 3.91 > 회사채 2.02", f"SELECT ROUND(AVG(CASE WHEN TRIM(std_pd_mcls_nm)='국공채' THEN dur END),2), ROUND(AVG(CASE WHEN TRIM(std_pd_mcls_nm)='특수채' THEN dur END),2), ROUND(AVG(CASE WHEN TRIM(std_pd_mcls_nm)='회사채' THEN dur END),2) {B} WHERE dur<>99", (3.97, 3.91, 2.02), 0.02)
chk("C-dur-3", "이론 위반 dur > 잔존/365+0.1 : 200", f"SELECT COUNT(*) {B} WHERE dur IS NOT NULL AND dur<>99 AND remaining_days IS NOT NULL AND dur > remaining_days/365.0 + 0.1", 200)
...
chk("C-ndydur", "ndy_dur 0값 72 · dur>0인데 0 : 20 · 99 3 · dur 과 상이 21,408", f"SELECT SUM(ndy_dur=0), SUM(ndy_dur=0 AND dur>0), SUM(ndy_dur=99), SUM(ndy_dur<>dur) {B}", (72, 20, 3, 21408))
chk("C-ndycov", "ndy_cov 0값 121 · cov>0인데 0 : 20 · cov 와 상이 21,612", f"SELECT SUM(ndy_cov=0), SUM(ndy_cov=0 AND cov>0), SUM(ndy_cov<>cov) {B}", (121, 20, 21612))
...
chk("C-ndyay", "ndy_applied_yield =applied 21,619 vs 21,817 (yaml 두 값 상충) · 위장결측 15", f"SELECT SUM(ndy_applied_yield=applied_yield), SUM(ndy_applied_yield=0 AND applied_yield>0) {B}", (21619, 15))
...
chk("C-corp", "corp 634 · pretax/buy 1.00~1.08 · after=pretax 31 · after/pretax 0.90~1.00", f"SELECT COUNT(corp_pretax_yield), ROUND(MIN(corp_pretax_yield/buy_yield),2), ROUND(MAX(corp_pretax_yield/buy_yield),2), SUM(corp_after_tax_yield=corp_pretax_yield), ROUND(MIN(corp_after_tax_yield/corp_pretax_yield),2), ROUND(MAX(corp_after_tax_yield/corp_pretax_yield),2) {B}", (634, 1.00, 1.08, 31, 0.90, 1.00), 0.01)
...
chk("C-pen-2", "pen Y 전부 공모 · 무등급 1,524 · 등급 있으면 A- 이상 (미만 0)", f"SELECT SUM(bd_ofr_tcd<>'공모'), SUM(crd_grd IS NULL OR TRIM(crd_grd)=''), SUM(TRIM(crd_grd) IN ('BBB+','BBB0','BBB-','BB0','BB-','B+','B-','C0')) {B} WHERE pd_pen_tr_yn='Y'", (0, 1524, 0))
# ── name_encoding ─────────────────────────────────────────────
...
chk("N-coco-grade", "코코 187 전부 AA급?", f"SELECT COUNT(*), SUM(TRIM(crd_grd) LIKE 'AA%'), SUM(crd_grd IS NULL OR TRIM(crd_grd)='') {B} WHERE pd_nm REGEXP '코코(?!리아)|조건부자본'", (187, 187, 0))
