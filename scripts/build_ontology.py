...
_LABEL_CONFLICTS: list = []     # (node_id, label) — emit_kg 가 채우고 report 가 적는다 (F1)
...
def emit_kg(con, shared):
    ...
    # KG 3R F1 — FundAttribute 라벨이 Region·AssetClass·Country 라벨 또는 상품군 명사·지역 수식어(라우터 PRODUCT/QUALIFIER)와 같으면
    #   Ground 매칭 키에서 빼고 alias(코드)로만 산다(provenance='label_conflict'). 'ETF'(M113)·'중국'(W축)·'국내'(V101)가 Region/상품군
    #   명사를 가로채던 KG-023·025·026 회귀. 충돌은 KG 실물에서 계산하고 리포트에 남긴다 — 이름 하드코딩 아님.
    sys.path.insert(0, PROJECT_ROOT)
    from src.runtime.router import PRODUCT as _PRODUCT, QUALIFIER as _QUALIFIER
    conflict_labels = {norm(k) for k in list(_PRODUCT) + list(_QUALIFIER)}
    for _f, _doc in shared.items():
        if _doc.get("entity") in ("Region", "AssetClass", "Country"):
            for _n in (_doc.get("nodes") or {}).values():
                for lab in (_n.get("label_ko"), _n.get("label_en")):
                    if lab:
                        conflict_labels.add(norm(lab))
    _LABEL_CONFLICTS.clear()
    ...
    for fname, doc in shared.items():
        ...
        for node_id, node in (doc.get("nodes") or {}).items():
            ...
            if entity == "FundAttribute" and node.get("label_ko") and norm(node["label_ko"]) in conflict_labels:
                provenance = "label_conflict"
                _LABEL_CONFLICTS.append((node_id, node["label_ko"]))
...
def report(con, enums, shared, distinct_cache, warnings):
    ...
    if _LABEL_CONFLICTS:
        lines.append("## FundAttribute 라벨 충돌 — 매칭 키 제외(alias 로만) · provenance=label_conflict (KG 3R F1)")
        lines.append("- 기준: Region·AssetClass·Country 라벨 · 라우터 상품군 명사(PRODUCT) · 지역 수식어(QUALIFIER) 와 정규화 라벨이 같음")
        lines += [f"- {nid}: '{lab}'" for nid, lab in _LABEL_CONFLICTS]
        lines.append("")
    pendings = []
