...
- 날짜 조건은 하이픈 없는 정수 YYYYMMDD 로 쓴다 (예: mat_dt >= 20260824). 2029-08-22 는 SQLite 에서 뺄셈(=1999)이 되고, '2029-08-22' 문자열은 REAL 컬럼과 비교가 깨진다. 만기 상한(mat_dt <= …)은 질문이 기간·연도·시점('3년 안에'·'2028년까지'·'내년')을 말할 때만 쓰고, 그때는 하한 mat_dt >= 20260824 을 함께 쓴다. 질문에 그런 말이 없으면 상한을 만들지 않는다 — '만기가 가장 긴' 은 정렬이지 창이 아니다. 날짜에 산술을 쓰지 않는다 — 20260824+90 은 SQLite 에서 정수 덧셈(20260914)이지 90일 뒤(20261122)가 아니다. 창의 양 끝은 미리 계산한 리터럴로만 적는다.
