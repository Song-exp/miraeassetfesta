...
def test_pipeline_regenerates_once_on_value_violation(ctx):
    ...
    # 재생성된 SQL 이 채택됐는지만 본다 — 이후 가드(신용등급 컬럼 보강·억원 병기 등)가 SELECT 를 넓힐 수 있어
    # 문자열 등호는 쓰지 않는다 (병철·리드 양쪽이 같은 취지로 고친 것을 병합)
    assert "crd_grd = 'AA-'" in r.sql and "AA-등급" not in r.sql
    assert "신용등급 AA-" in r.answer and "[Answer] 채권 목록 답변 기계 조립" in r.think_trace   # 2026-09-02: 채권 목록은 기계 조립
